...
- **Vercel Deployment Sync**: Merged all changes from the `prod` branch to the `main` branch on the Motherboard repository and pushed to GitHub, triggering a clean production deployment on Vercel.
...
- **Programmatic Activities API (`server.js`):** Added a secure, API key-authorized POST endpoint `/api/v1/activities` on `cal.gobitsnbytes.org` (port 3100) allowing external systems to log pulses, events, reports, and partnerships programmatically and allocate points to Notion nodes, complete with automated Discord embeds.
- **Verification**: Verified that all 210 bot tests and 93 python backend tests pass 100% green. Tested the fallback and new Activities API endpoint directly on the VPS via curl, verifying successful point allocation and bot embeds. Restarted all services.
