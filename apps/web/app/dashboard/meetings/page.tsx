...
export default function MeetingsPage() {
  ...
  async function createConnection(event: FormEvent<HTMLFormElement>) {
    ...
    await run("connection", async () => {
      const host = String(form.get("user_id"));
      const connection = host === "shared-core"
        ? { shared_host_name: "bits&bytes", shared_host_email: "gobitsnbytes@gmail.com", api_key: form.get("api_key") }
        : { user_id: host, api_key: form.get("api_key") };
      await api("/api/calendar/connections/calcom", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(connection) });
  ...
  return <main className="mx-auto max-w-7xl space-y-6 pb-12">
    ...
    {!loading && tab === "connections" && <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]" aria-labelledby="connections-title">
      ...
      <form onSubmit={createConnection} className="h-fit space-y-4 rounded-base border-2 border-border bg-secondary-background p-5"><div><h3 className="font-black">Connect a host</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Use a dedicated Cal.com API key after Google Calendar and Google Meet are connected. Save, then click Verify to register its webhook.</p></div><label className="block text-sm font-bold">Host<select name="user_id" required className={`${fieldClass} mt-1`}><option value="">Select a host</option><option value="shared-core">bits&amp;bytes — gobitsnbytes@gmail.com (shared)</option>{users.filter((user) => user.email?.toLowerCase() !== "gobitsnbytes@gmail.com").map((user) => <option key={user.id} value={user.id}>{user.display_name}{user.email ? ` — ${user.email}` : ""}</option>)}</select></label><label className="block text-sm font-bold">Cal.com API key<input name="api_key" type="password" required minLength={8} autoComplete="new-password" className={`${fieldClass} mt-1`} placeholder="cal_live_…" /></label><button className={`${primaryButton} w-full`} disabled={busy === "connection"}>{busy === "connection" ? <Loader2 className="size-4 animate-spin motion-reduce:animate-none" /> : <Plus className="size-4" />} Save connection</button></form>
    </section>}
