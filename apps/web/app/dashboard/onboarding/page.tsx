...
export default function OnboardingDashboardPage() {
  ...
  const resendInvite = async (caseId: string, participantId: string) => {
    ...
    try {
      const response = await fetch(`/api/onboarding/cases/${caseId}/participants/${participantId}/resend`, { method: "POST" });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(typeof body.detail === "string" ? body.detail : "Could not resend the invite.");
      setMessage(body.email_sent ? `A fresh onboarding invite was sent to ${body.email}. Older links are now invalid.` : `The portal link was refreshed for ${body.email}, but SMTP is not configured to send it.`);
      await load();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Could not resend the invite.");
    } finally {
