...
class Settings(BaseSettings):
    ...
    api_service_user_id: str | None = Field(
        default=None, validation_alias="API_SERVICE_USER_ID"
    )
    nextauth_secret: str = Field(validation_alias="NEXTAUTH_SECRET")
    nextauth_url: str = Field(
        default="http://localhost:3000", validation_alias="NEXTAUTH_URL"
    )
    api_url: str = Field(default="http://localhost:8000", validation_alias="API_URL")
    calendar_webhook_base_url: str | None = Field(
    ...
    sync_interval_minutes: int = Field(
        default=15, validation_alias="SYNC_INTERVAL_MINUTES"
    )
    enable_sync_scheduler: bool = Field(
        default=False, validation_alias="ENABLE_SYNC_SCHEDULER"
    )
    # RazorpayX API key — optional until real banking integration is wired
    razorpayx_api_key: str | None = Field(
        default=None, validation_alias="RAZORPAYX_API_KEY"
    )
    ...
    razorpayx_key_id: str | None = Field(
        default=None, validation_alias="RAZORPAYX_KEY_ID"
    )
    ...
    smtp_user: str | None = Field(
        default="legal@gobitsnbytes.org", validation_alias="SMTP_USER"
    )
    smtp_pass: str | None = Field(default=None, validation_alias="SMTP_PASS")
    smtp_from: str = Field(
        default="bits&bytes Legal <legal@gobitsnbytes.org>",
    ...
    sparkcloud_join_url: str = Field(
        default="https://sparkden.org/org/join/HJ6POGpRWhZRvT8jtEYqmMpMhmLiMOEo",
        validation_alias="SPARKCLOUD_JOIN_URL",
    )

    # SparkCloud AI Proxy settings
    sparkcloud_api_key: str | None = Field(
        default=None, validation_alias="SPARKCLOUD_API_KEY"
    )
    sparkcloud_base_url: str = Field(
        default="https://cloud.sparkden.org/api/ai/v1",
        validation_alias="SPARKCLOUD_BASE_URL",
    )
    sparkcloud_model: str = Field(default="auto", validation_alias="SPARKCLOUD_MODEL")

    # Inbound Email Webhook Security
    inbound_email_webhook_secret: str | None = Field(
        default=None, validation_alias="INBOUND_EMAIL_WEBHOOK_SECRET"
    )
    ...
    legal_inbox_imap_host: str | None = Field(
        default=None, validation_alias="LEGAL_INBOX_IMAP_HOST"
    )
    legal_inbox_imap_port: int = Field(
        default=993, validation_alias="LEGAL_INBOX_IMAP_PORT"
    )
    legal_inbox_imap_user: str | None = Field(
        default=None, validation_alias="LEGAL_INBOX_IMAP_USER"
    )
    legal_inbox_imap_password: str | None = Field(
        default=None, validation_alias="LEGAL_INBOX_IMAP_PASSWORD"
    )
    legal_inbox_mailbox: str = Field(
        default="INBOX", validation_alias="LEGAL_INBOX_MAILBOX"
    )
    legal_inbox_poll_seconds: int = Field(
        default=60, validation_alias="LEGAL_INBOX_POLL_SECONDS"
    )
    legal_nudge_enabled: bool = Field(
        default=True, validation_alias="LEGAL_NUDGE_ENABLED"
    )
    legal_org_mailbox: str = Field(
        default="legal@gobitsnbytes.org", validation_alias="LEGAL_ORG_MAILBOX"
    )
    legal_retrieval_backend: str = Field(
        default="qenlo", validation_alias="LEGAL_RETRIEVAL_BACKEND"
    )
    legal_qenlo_data_dir: str = Field(
        default="/var/lib/bnb/legal-qenlo", validation_alias="LEGAL_QENLO_DATA_DIR"
    )
    legal_qenlo_dimension: int = Field(
        default=256, validation_alias="LEGAL_QENLO_DIMENSION"
    )
