...
def test_okf_knowledge_store_and_rule_engine():
    ...
    findings = rule_engine.evaluate_clause(
        "§8.1",
        "Limitation of Liability",
        "Vendor shall provide uncapped liability for all damages.",
    )
    assert len(findings) >= 1
    assert findings[0]["severity"] == "high"

    # Test payment terms threshold
    payment_findings = rule_engine.evaluate_clause(
        "§3.2", "Payment Terms", "Invoices shall be payable within Net 90 days."
    )
...
async def test_contract_assistant_rules_endpoint(client):
...
async def test_contract_assistant_autofix_endpoint(super_admin: User, client):
    payload = {
    ...
    response = await request_as(
        client, super_admin.id, "POST", "/api/contract-assistant/autofix", json=payload
    )
    assert response.status_code == 200
    data = response.json()
    assert data["tier"] == 1
    assert (
        ...
        in data["suggested_rewrite"]
    )


async def test_inbound_email_webhook_security(monkeypatch, client):
...
async def test_contract_assistant_database_crud_and_dispatch(
    super_admin: User, db_session: AsyncSession, client
):
    ...
    dispatch_blocked_res = await client.post(
        ...
        json={
            ...
            "recipients": [
                {"name": "Akshat", "email": "akshat@gobitsnbytes.org", "role": "signer"}
            ],
        },
    )
    ...
    patch_res = await client.patch(
        ...
        json={
            "status": "resolved",
            "suggested_rewrite": "Liability is capped at ₹5,00,000.",
        },
    )
    ...
    export_res = await client.get(
        f"/api/contract-assistant/contracts/{c.id}/export-void"
    )
