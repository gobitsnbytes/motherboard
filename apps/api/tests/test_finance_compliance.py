...
async def test_self_approval_forbidden_on_approve_and_reject(
    ...
    create_res = await _create_request(client, requester, account.id, 50_000)
    assert create_res.status_code == 201
    request_id = create_res.json()["id"]

    approve_res = await _review(client, requester, request_id, "approve")
    assert approve_res.status_code == 403

    reject_res = await _review(client, requester, request_id, "reject")
...
async def test_small_request_single_approval_moves_funds(
    ...
    create_res = await _create_request(
        client, requester, account.id, DEFAULT_THRESHOLD_PAISE - 1
    )
    assert create_res.status_code == 201
    body = create_res.json()
    assert body["dual_approval_required"] is False

    approve_res = await _review(client, approver, body["id"], "approve")
...
async def test_large_request_requires_two_distinct_approvers(
    ...
    create_res = await _create_request(
        client, requester, account.id, DEFAULT_THRESHOLD_PAISE
    )
    body = create_res.json()
    request_id = body["id"]
    ...
    first = await _review(client, approver_a, request_id, "approve")
    ...
    dup = await _review(client, approver_a, request_id, "approve")
    assert dup.status_code == 200
    assert dup.json()["status"] == "pending"
    assert dup.json()["approvals_count"] == 1

    # Progress visible on GET endpoints
    got = await request_as(
        client, super_admin.id, "GET", f"/api/finance/requests/{request_id}"
    )
    assert got.status_code == 200
    assert got.json()["approvals_count"] == 1

    # Second distinct approver completes the commitment
    second = await _review(client, approver_b, request_id, "approve")
    ...
    txns = await db_session.execute(
        select(VirtualTransaction).where(
            VirtualTransaction.reference_type == "money_request"
        )
    )
...
async def test_dual_approval_disabled_moves_funds_immediately(
    db_session: AsyncSession, super_admin: User, monkeypatch, client
):
    ...
    try:
        ...
        create_res = await _create_request(
            client, requester, account.id, DEFAULT_THRESHOLD_PAISE * 2
        )
        request_id = create_res.json()["id"]

        approve_res = await _review(client, approver, request_id, "approve")
        assert approve_res.status_code == 200
        assert approve_res.json()["status"] == "approved"
...
async def test_compliance_endpoint_shape(
    db_session: AsyncSession, super_admin: User, client
):
    res = await request_as(client, super_admin.id, "GET", "/api/finance/compliance")
    ...
    assert (
        data["dual_auth_threshold_paise"]
        == settings.finance_dual_approval_threshold_paise
    )
...
async def test_compliance_counts_open_requests_above_threshold(
    ...
    before = await request_as(client, super_admin.id, "GET", "/api/finance/compliance")
    ...
    after = await request_as(client, super_admin.id, "GET", "/api/finance/compliance")
    data = after.json()
...
async def test_audit_trail_written_for_request_lifecycle_and_card_charge(
    ...
    create_res = await _create_request(client, requester, account.id, 25_00)
    request_id = create_res.json()["id"]
    created_rows = await _audit_rows(db_session, "finance.request.created", request_id)
    assert len(created_rows) == 1
    assert created_rows[0].target_type == "money_request"

    reject_res = await _review(client, reviewer, request_id, "reject")
    ...
    create_res2 = await _create_request(client, requester, account.id, 10_00)
    request_id2 = create_res2.json()["id"]
    approve_res = await _review(client, reviewer, request_id2, "approve")
    assert approve_res.status_code == 200
    ...
    card_res = await request_as(
        client, super_admin.id, "POST", "/api/finance/cards", json=card_payload
    )
...
async def test_fy_export_streams_csv(
    db_session: AsyncSession, super_admin: User, client
):
    ...
    export_res = await request_as(
        client, super_admin.id, "GET", f"/api/finance/reports/fy?fy={fy}"
    )
...
async def test_fy_export_validates_format_and_span(
    ...
    for bad in ["2026", "2026/27", "abcd-ef", "2026-28", "2026-270"]:
        res = await request_as(
            client, super_admin.id, "GET", f"/api/finance/reports/fy?fy={bad}"
        )
        assert res.status_code == 400, f"{bad} should be rejected"


async def test_fy_export_requires_finance_admin(db_session: AsyncSession, client):
    plain = await _make_user(db_session, "No Finance Admin", super_admin=False)
    res = await request_as(
        client, plain.id, "GET", "/api/finance/reports/fy?fy=2026-27"
    )
