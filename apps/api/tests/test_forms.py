...
async def test_forms_validate_answers_and_return_labeled_responses(super_admin, client):
    ...
    invalid = await client.post(
        ...
        headers={"Idempotency-Key": "invalid-answer-key-001"},
        json={"answers": {"unknown": "value"}, "accepted_terms": True},
    )
    ...
    submitted = await client.post(
        "/api/forms/public/interest-form/submissions",
        headers=headers,
        json={"answers": {"name": "Asha", "track": "Code"}, "accepted_terms": True},
    )
    ...
    mismatched = await client.post(
        ...
        headers=headers,
        json={
            "answers": {"name": "Someone else", "track": "Design"},
            "accepted_terms": True,
        },
    )
    assert mismatched.status_code == 409

    responses = await request_as(
        client, super_admin_id, "GET", f"/api/forms/{form_id}/submissions"
    )
    ...
    exported = await request_as(
        client, super_admin_id, "GET", f"/api/forms/{form_id}/submissions/export.csv"
    )
...
async def test_form_lists_and_exports_only_canonical_submissions(
    super_admin, db_session: AsyncSession, client
):
    ...
    created = await request_as(
        ...
        "/api/forms",
        json={
            "title": "Deduplicated form",
            "slug": "deduplicated-form",
            "blocks": blocks,
            "is_published": True,
        },
    )
    ...
    first = await client.post(
        "/api/forms/public/deduplicated-form/submissions",
        headers={"Idempotency-Key": "canonical-answer-key-001"},
        json={"answers": {"name": "Asha"}, "accepted_terms": True},
    )
    repeated = await client.post(
        "/api/forms/public/deduplicated-form/submissions",
        headers={"Idempotency-Key": "duplicate-answer-key-001"},
        json={"answers": {"name": "Asha"}, "accepted_terms": True},
    )
    duplicate = await db_session.get(
        PublicFormSubmission, uuid.UUID(repeated.json()["id"])
    )
    duplicate.duplicate_of_id = uuid.UUID(first.json()["id"])
    await db_session.commit()

    responses = await request_as(
        client, super_admin.id, "GET", f"/api/forms/{form_id}/submissions"
    )
    assert [row["id"] for row in responses.json()] == [first.json()["id"]]
    exported = await request_as(
        client, super_admin.id, "GET", f"/api/forms/{form_id}/submissions/export.csv"
    )
    assert len(list(csv.reader(io.StringIO(exported.text.lstrip("\ufeff"))))) == 2


async def test_form_uploads_require_token_and_are_idempotent(
    super_admin, tmp_path, monkeypatch, client
):
