...
async def test_plain_member_can_create_a_company(db_session: AsyncSession, client):
...
async def test_duplicate_name_without_website_returns_409(
    ...
    first = await request_as(
        client, user.id, "POST", "/api/dyslexic/companies", json={"name": "Swiggy"}
    )
    assert first.status_code == 201
...
async def test_list_filters_by_stage_and_query(db_session: AsyncSession, client):
    user = await make_volunteer(db_session)
    ...
    by_stage = await request_as(
        client, user.id, "GET", "/api/dyslexic/companies?stage=sponsored"
    )
    by_query = await request_as(
        client, user.id, "GET", "/api/dyslexic/companies?q=swig"
    )
...
async def test_archived_companies_are_hidden_by_default(
    db_session: AsyncSession, client
):
    user = await make_volunteer(db_session)
    ...
    await request_as(
        ...
        user.id,
        "PATCH",
        f"/api/dyslexic/companies/{company_id}",
        json={"is_archived": True},
    )

    default_list = await request_as(client, user.id, "GET", "/api/dyslexic/companies")
    with_archived = await request_as(
        client, user.id, "GET", "/api/dyslexic/companies?archived=true"
    )
...
async def test_companies_cannot_be_deleted(db_session: AsyncSession, client):
    """
...
async def test_company_detail_includes_contacts_and_timeline(
    db_session: AsyncSession, client
):
    user = await make_volunteer(db_session)
    ...
    detail = await request_as(
        client, user.id, "GET", f"/api/dyslexic/companies/{company_id}"
    )
...
async def test_unauthenticated_requests_are_rejected(client):
    ...
    response = await client.get("/api/dyslexic/companies")
