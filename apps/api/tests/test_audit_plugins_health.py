...
@pytest.mark.asyncio
async def test_audit_logs_list_and_filter(db_session: AsyncSession, super_admin: User):
    # Create actor
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        # List all (limit = 50)
        response = await request_as(ac, super_admin.id, "GET", "/api/audit/")
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 3

        # Limit and offset
        response = await request_as(ac, super_admin.id, "GET", "/api/audit/?limit=1")
        assert response.status_code == 200
        assert len(response.json()) == 1

        # Filter by action
        response = await request_as(ac, super_admin.id, "GET", "/api/audit/?action=test.create")
        assert response.status_code == 200
        assert len(response.json()) == 1
        assert response.json()[0]["action"] == "test.create"

        # Filter by actor_id
        response = await request_as(ac, super_admin.id, "GET", f"/api/audit/?actor_id={actor.id}")
        assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_plugin_registry_endpoints(db_session: AsyncSession, super_admin: User):
    # Seed a plugin
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        # List plugins
        response = await request_as(ac, super_admin.id, "GET", "/api/plugins/")
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 1
        ...
        # Get specific plugin
        response = await request_as(ac, super_admin.id, "GET", f"/api/plugins/{plugin.id}")
        assert response.status_code == 200
        assert response.json()["version"] == "1.2.3"
        assert response.json()["config"] == {"theme": "dark"}

        # Get nonexistent plugin (404)
        response = await request_as(ac, super_admin.id, "GET", "/api/plugins/nonexistent.id")
        assert response.status_code == 404

        # Update plugin settings
        payload = {"is_enabled": False, "config": {"theme": "light"}}
        response = await request_as(ac, super_admin.id, "PATCH", f"/api/plugins/{plugin.id}", json=payload)
        assert response.status_code == 200
