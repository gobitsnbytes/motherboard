...
async def test_can_super_admin(db_session: AsyncSession):
    principal = ResolvedPrincipal(
        user_id=uuid.uuid4(), group_ids=[], is_super_admin=True
    )
...
async def test_can_user_direct_grant(db_session: AsyncSession):
    ...
    grant = Grant(
        principal_type="user", principal_id=user_id, permission_key="test.read"
    )
...
async def test_can_group_grant(db_session: AsyncSession):
    ...
    group_id = uuid.uuid4()
    principal = ResolvedPrincipal(
        user_id=user_id, group_ids=[group_id], is_super_admin=False
    )

    grant = Grant(
        principal_type="group", principal_id=group_id, permission_key="group.action"
    )
    db_session.add(grant)
    await db_session.commit()
...
async def test_batch_can(db_session: AsyncSession):
    ...
    grant1 = Grant(
        principal_type="user", principal_id=user_id, permission_key="batch.read"
    )
    grant2 = Grant(
