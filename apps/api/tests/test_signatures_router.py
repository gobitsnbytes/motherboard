...
def sample_pdf_bytes():
    ...
    doc.build(
        [
            Paragraph("Test Contract Document Header", styles["Heading1"]),
            Paragraph(
                "This is a sample legal contract for digital signature verification.",
                styles["Normal"],
            ),
        ]
    )
...
async def test_upload_contract_pdf(sample_pdf_bytes: bytes, super_admin, client):
    files = {"file": ("contract.pdf", sample_pdf_bytes, "application/pdf")}
    response = await request_as(
        client, super_admin.id, "POST", "/api/signatures/upload", files=files
    )
...
async def test_signature_request_creation_and_signing_flow(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
    ...
    upload_res = await request_as(
        client, user.id, "POST", "/api/signatures/upload", files=files
    )
    ...
    create_res = await request_as(
        client, user.id, "POST", "/api/signatures/requests", json=payload
    )
    ...
    sign_res = await client.post(
        ...
        json={"fields": [{"field_id": field_id, "value": signature_value}]},
    )
...
async def test_public_verification_not_found(client):
    res = await client.get("/api/signatures/verify/non_existent_hash_123")
    assert res.status_code == 404


async def test_signature_request_otp_verification_flow(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
    get_settings().smtp_host = None
    ...
    bad_req = await client.post(
        f"/api/signatures/sign/{recipient_token}/request-otp",
    ...
    bypass_sign = await client.post(
        f"/api/signatures/sign/{recipient_token}", json={"fields": []}
    )
    ...
    with patch("app.routers.signatures.secrets.randbelow", return_value=234567):
        good_req = await client.post(
            f"/api/signatures/sign/{recipient_token}/request-otp",
            json={"email": "akshatkushwah@gmail.com"},
        )
    ...
    res = await db_session.execute(
        select(SignatureRecipient).where(
            SignatureRecipient.access_token == recipient_token
        )
    )
    ...
    for _ in range(5):
        wrong_verify = await client.post(
            f"/api/signatures/sign/{recipient_token}/verify-otp", json={"otp": "000000"}
        )
    ...
    right_verify = await client.post(
        f"/api/signatures/sign/{recipient_token}/verify-otp", json={"otp": "334567"}
    )
    assert right_verify.status_code == 200
    assert right_verify.json()["success"] is True


async def test_contract_compliance_check(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
...
async def test_file_upload_verification(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
...
async def test_void_signature_request_blocks_signing(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
    ...
    otp_res = await client.post(
        f"/api/signatures/sign/{token}/request-otp",
        json={"email": "signer.void@example.com"},
    )
    ...
    sign_res = await client.post(
        ...
        json={
            "fields": [{"field_id": field_id, "value": "data:image/png;base64,sample"}]
        },
    )
...
async def test_delete_signature_request_permanently(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
...
async def test_audit_trail_endpoint_and_response_inclusion(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
    ...
    audit_res = await request_as(
        client, user.id, "GET", f"/api/signatures/requests/{req_id}/audit"
    )
...
async def test_org_countersign_flow(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
    ...
    cs_res = await request_as(
        ...
        user.id,
        "POST",
        f"/api/signatures/requests/{req_data['id']}/countersign",
    ...
    rows = (
        (
            await db_session.execute(
                _sel(_SAL).where(_SAL.request_id == uuid.UUID(req_data["id"]))
            )
        )
        .scalars()
        .all()
    ...
    audit_actions = [e["action"] for e in vdata["audit_trail"]] + [
...
async def test_countersign_rejected_without_permission(
    db_session: AsyncSession, sample_pdf_bytes: bytes, client
):
    ...
    upload_res = await request_as(
        client, admin.id, "POST", "/api/signatures/upload", files=files
    )
    file_path = upload_res.json()["file_path"]
    ...
    create_res = await request_as(
        client, admin.id, "POST", "/api/signatures/requests", json=payload
    )
    req_id = create_res.json()["id"]
    ...
    denied = await request_as(
        ...
        plain.id,
        "POST",
        f"/api/signatures/requests/{req_id}/countersign",
        json={"note": None},
    )
