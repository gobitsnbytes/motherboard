...
async def test_sync_endpoints_require_auth(client):
    ...
    response = await client.get("/api/sync/runs")
    ...
    response = await client.get(
        "/api/sync/runs", headers={"X-User-Id": str(uuid.uuid4())}
    )
    assert response.status_code == 401
    assert "Missing internal authentication headers" in response.json()["detail"]
    ...
    response = await client.get(
        ...
        headers=internal_auth_headers(
            "invalid-uuid", method="GET", path="/api/sync/runs"
        ),
    )
...
async def test_sync_endpoints_permission_check(db_session: AsyncSession, client):
    ...
    # Read runs
    response = await request_as(client, user.id, "GET", "/api/sync/runs")
...
async def test_sync_runs_list_and_get(db_session: AsyncSession, client):
    # Create super admin
    ...
    # Get run by ID
    response = await request_as(client, user.id, "GET", f"/api/sync/runs/{run1.id}")
    assert response.status_code == 200
    ...
    response = await request_as(
        client, user.id, "GET", f"/api/sync/runs/{uuid.uuid4()}"
    )
    assert response.status_code == 404


async def test_trigger_sync_authorized(db_session: AsyncSession, client):
