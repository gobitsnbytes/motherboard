...
async def test_api_key_auth_header_success(
    ...
    response = await client.get("/api/meetings/", headers={"X-API-Key": api_key})
    # Should authenticate successfully as the super admin context and return list of meetings
    assert response.status_code == 200
    assert isinstance(response.json(), list)


async def test_api_key_auth_bearer_success(
    db_session: AsyncSession, monkeypatch, client
):
    service_user = User(display_name="Meeting Service")
    permission = Permission(key="meetings.read")
    ...
    response = await client.get(
        "/api/meetings/", headers={"Authorization": f"Bearer {api_key}"}
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)


async def test_api_key_auth_invalid(db_session: AsyncSession, client):
    response = await client.get(
        "/api/meetings/", headers={"X-API-Key": "invalid-secret-key-123"}
    )
    assert response.status_code == 401
    assert response.json()["detail"] == "Invalid API Key"


async def test_api_key_auth_rejects_mixed_credentials(db_session: AsyncSession, client):
    response = await client.get(
        "/api/meetings/",
        headers={
...
async def test_api_key_auth_requires_explicit_service_identity(
...
async def test_api_key_auth_rejects_super_admin_service_identity(
...
async def test_api_key_auth_rejects_inactive_service_identity(
    db_session: AsyncSession, monkeypatch, client
):
...
async def test_api_key_auth_missing(db_session: AsyncSession, client):
