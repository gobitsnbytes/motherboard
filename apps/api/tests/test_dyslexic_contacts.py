...
@pytest.fixture
async def setup(db_session: AsyncSession, client):
...
async def test_create_contact_lowercases_email_and_advances_stage(setup, client):
...
async def test_same_email_twice_at_one_company_is_refused(setup, client):
...
async def test_same_person_at_another_company_warns_but_succeeds(setup, client):
...
async def test_claim_conflict_names_the_holder(setup, client):
...
async def test_claim_can_be_released_and_retaken(setup, client):
    contact = await request_as(
        client,
        setup["priya"].id,
        "POST",
    ...
    await request_as(
        client, setup["priya"].id, "POST", f"/api/dyslexic/contacts/{contact_id}/claim"
    )
...
async def test_expired_claim_frees_the_contact(setup, db_session: AsyncSession, client):
...
async def test_contacts_cannot_be_deleted(setup, client):
    contact = await request_as(
        client,
        setup["priya"].id,
        "POST",
