...
def _annotated_paragraphs(document_key: str) -> list[str]:
    source = (
        ...
        / "templates"
        / TEMPLATE_MANIFEST[document_key]["template"]
    )
...
def test_every_marker_appears_exactly_once(document_key: str) -> None:
    ...
    for marker in signature_markers(document_key).values():
        assert body.count(marker) == 1, (
            f"{document_key}: {marker} is not a unique PDF anchor"
        )


@pytest.mark.parametrize("document_key", FORM_KEYS)
def test_signatures_land_on_a_clear_line_under_their_own_block(
    document_key: str,
) -> None:
    ...
    for field in fields_for(document_key):
        ...
        assert paragraphs[index] == marker, (
            f"{document_key}: {marker} shares a line with {paragraphs[index]!r}"
        )
...
def test_printed_name_accompanies_the_signature(document_key: str) -> None:
    paragraphs = _annotated_paragraphs(document_key)
    assert any(
        text.startswith("Full Name (Print):") and "Test Person" in text
...
def test_auto_fields_are_hidden_from_editors(document_key: str) -> None:
    ...
    auto = {
        field.id for field in FORM_FIELDS[document_key] if field.editable_by == "auto"
    }
...
def test_multiline_values_get_their_own_paragraph() -> None:
    ...
    assert paragraphs[index] == label, (
        "prose was tab-appended to its label and will wrap into the margin"
    )
...
def test_markers_become_boxes_and_are_erased(tmp_path: Path) -> None:
    ...
    for _field, placement in placements:
        assert (
            placement["width"] > 10 and placement["height"] > 3
        )  # per cent of the page
        assert (
            0 <= placement["pos_x"] and placement["pos_x"] + placement["width"] <= 100
        )
        assert (
            0 <= placement["pos_y"] and placement["pos_y"] + placement["height"] <= 100
        )
