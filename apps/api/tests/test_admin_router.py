...
from app.db.models import User, SyncRun
from conftest import request_as


async def test_admin_endpoints_permission_check(db_session: AsyncSession, client):
    ...
    # Reset cache
    response = await request_as(client, user.id, "POST", "/api/admin/reset-cache")
    ...
    # Clear sync state
    response = await request_as(client, user.id, "POST", "/api/admin/clear-sync-state")
    assert response.status_code == 403


async def test_admin_reset_cache_no_redis(db_session: AsyncSession, client):
...
async def test_admin_reset_cache_success(db_session: AsyncSession, client):
    user = User(display_name="Super Admin", is_super_admin=True)
    db_session.add(user)
    await db_session.commit()
...
async def test_admin_clear_sync_state(db_session: AsyncSession, client):
    user = User(display_name="Super Admin", is_super_admin=True)
    db_session.add(user)
...
async def test_admin_rebuild_permissions(db_session: AsyncSession, client):
    user = User(display_name="Super Admin", is_super_admin=True)
    db_session.add(user)
    await db_session.commit()
